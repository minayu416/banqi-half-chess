...
import { easyComputer } from "./computer";
...
function Board({
  ...
  const showOpponentSide =
    gameId === "single" ? opponent.displayName : opponent.uid;
  ...
  useEffect(() => {
    if (gameId === "single" && sequence === opponent.displayName) {
      const timer = setTimeout(() => {
        easyComputer(shuffledChess, side["Oppo."], rules, emitChange);
      }, 1000); // 模擬思考時間
  ...
  function emitChange(activeData, overData) {
    ...
    if (!activeData.chess.turned) {
      ...
      msg = `${t("game.eventMessage.turnOn")} [${
        String(i18nChessMapping[lng][activeData.chess.sn[0]]) ?? ""
      }] ${
        String(
          i18nChessMapping[lng].chess[activeData.chess.sn[0]][
            activeData.chess.type
          ]
        ) ?? ""
      }`;
    } else {
      msg =
        overData.chess === "."
          ? `${t("game.eventMessage.Moved")} [${
            ...
            } ${t("game.eventMessage.toEmptyPlace")}`
          : `[${String(i18nChessMapping[lng][activeData.chess.sn[0]]) ?? ""}] ${
              String(
                i18nChessMapping[lng].chess[activeData.chess.sn[0]][
                  activeData.chess.type
              ...
              String(i18nChessMapping[lng][overData.chess.sn[0]]) ?? ""
            }] ${
              String(
                i18nChessMapping[lng].chess[overData.chess.sn[0]][
                  overData.chess.type
                ]
              ) ?? ""
            }`;
      // TODO: can re-design the logic of using the index or position to calculate.
      updatedChess[activeData.position] = ".";
      updatedChess[overData.position] = activeData.chess;
      updatedChess[overData.position].position = overData.position;
    }
  ...
  function handleDragEnd(event) {
    ...
    if (!activeData.chess.turned) {
      // TODO: 重構這行 跟 emitChange，大部分的邏輯已經合併了
      const updatedChess = [...shuffledChess];
      const turnedChess = { ...activeData.chess };
      turnedChess.turned = true;
      updatedChess[activeData.position] = turnedChess;
      const msg = `${t("game.eventMessage.turnOn")} [${
        String(i18nChessMapping[lng][activeData.chess.sn[0]]) ?? ""
      }] ${
        String(
          i18nChessMapping[lng].chess[activeData.chess.sn[0]][
    ...
    if (rules.isMoveToEmptyPlace(overData)) {
      ...
      if (
        Math.abs(overData.position - activeData.position) === 1 ||
        Math.abs(overData.position - activeData.position) === 8
      ) {
...
function GameSection({ setEventInfo, eventInfo, params }) {
  ...
  useEffect(() => {
    if (gameId === "single") {
      ...
      const opponentSide = {
        uid: "@single18732",
        displayName: t("game.opponentName"),
      };
      setCurrentUser(me);
    ...
    <>
      ...
      >
        ...
        <p className="font-bold" style={{ color: "#96602E" }}>
          1. {t("instruction.rule1")}
        </p>
        <p className="font-bold" style={{ color: "#96602E" }}>
          2. {t("instruction.rule2")}
        </p>
        <p className="font-bold" style={{ color: "#96602E" }}>
          3. {t("instruction.rule3")}
        </p>
        <p className="font-bold" style={{ color: "#96602E" }}>
          4. {t("instruction.rule4")}
        </p>
        <p className="font-bold" style={{ color: "#96602E" }}>
          5. {t("instruction.rule5")}
        </p>
      </div>
    </>
  );
...
export default function Page({ params }) {
  ...
  return (
    <>
      ...
      {params.game !== "single" && showChatRoom && (
        <Sidebar
          lng={params.lng}
          gameId={params.game}
